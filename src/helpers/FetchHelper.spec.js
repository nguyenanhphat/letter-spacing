...
describe('fetchHelper.jsonToQueryString()', () => {
  ...
  test('json object that has nested json object', () => {
    const result = fetchHelper.jsonToQueryString({
      filters: {
        actual_drop_date: { start_date: '2018-09-30', end_date: '2018-10-30' },
        mail_class: ['standard', 'first_class', 'non_profit']
      }
    });

    expect(result).toBe(
      'filters=%7B%22actual_drop_date%22%3A%7B%22start_date%22%3A%222018-09-30%22%2C%22end_date%22%3A%222018-10-30%22%7D%2C%22mail_class%22%3A%5B%22standard%22%2C%22first_class%22%2C%22non_profit%22%5D%7D'
    );
  });
});
...
test('mergeWithDefaultHeaders()', () => {
  const headerObj = { x: 'y' };
  global.Headers.prototype.forEach = Object.entries(headerObj).forEach;
  const headers = new Headers({ x: 'y' });

  mergeWithDefaultHeaders(headers, { a: 'b' });
});

test('applyBeforeRequestInterceptors', () => {
...
test('applyAfterResponseInterceptors', () => {
  const originConsoleError = console.error;
  console.error = jest.fn();
  applyAfterResponseInterceptors({
    response: {},
